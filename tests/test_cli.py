...
from schema_agents.cli import load_model
from schema_agents.local_python_executor import LocalPythonExecutor
from schema_agents.models import HfApiModel, LiteLLMModel, OpenAIServerModel, TransformersModel
...
def test_load_model_litellm_model():
    with patch("schema_agents.cli.LiteLLMModel") as MockLiteLLMModel:
        MockLiteLLMModel.return_value = MagicMock()
...
def test_cli_main(capsys):
    with patch("schema_agents.cli.load_model") as mock_load_model:
        mock_load_model.return_value = "mock_model"
        with patch("schema_agents.cli.CodeAgent") as mock_code_agent:
            from schema_agents.cli import main
...
def test_vision_web_browser_main():
    with patch("schema_agents.vision_web_browser.helium"):
        with patch("schema_agents.vision_web_browser.load_model") as mock_load_model:
            mock_load_model.return_value = "mock_model"
            with patch("schema_agents.vision_web_browser.CodeAgent") as mock_code_agent:
                from schema_agents.vision_web_browser import helium_instructions, main
    ...
    # We don't need to test the actual execution of the LocalPythonExecutor here
    # since it's an async function and would require a more complex test setup
