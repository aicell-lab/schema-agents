...
docker = pytest.importorskip("docker", reason="Docker package not installed: `pip install 'schema_agents[docker]'`")

from PIL import Image

from schema_agents.monitoring import AgentLogger, LogLevel
from schema_agents.remote_executors import DockerExecutor, E2BExecutor
...
class TestE2BExecutor:
    @pytest.mark.asyncio
    async def test_e2b_executor_instantiation(self):
        pytest.skip("E2B module not found: `pip install 'schema-agents[e2b]'`")
        logger = MagicMock()
...
class TestDockerExecutor:
    ...
    async def test_state_persistence(self):
        ...
        await self.executor(code_action)

        code_action = "print(np.sqrt(a))"
        result, logs, final_answer = await self.executor(code_action)
    ...
    @pytest.mark.asyncio
    async def test_execute_image_output(self):
