...
from schema_agents.default_tools import BASE_PYTHON_TOOLS
from schema_agents.local_python_executor import (
    DANGEROUS_FUNCTIONS,
    DANGEROUS_MODULES,
    InterpreterError,
    LocalPythonExecutor,
...
"""
    ...
    def test_while(self):
        ...
        # test infinite loop
        code = "i = 0\nwhile i < 3:\n    i -= 1\ni"
        with patch("schema_agents.local_python_executor.MAX_WHILE_ITERATIONS", 100):
...
class TestLocalPythonExecutor:
    ...
    @pytest.mark.asyncio
    async def test_call_from_dict(self, code):
        executor = LocalPythonExecutor([])
        result, _, _ = await executor(code)
        assert result == 11
    ...
    @pytest.mark.asyncio
    async def test_chained_assignments(self, code):
        executor = LocalPythonExecutor([])
        await executor.send_tools({})
        result, _, _ = await executor(code)
        assert result == 1

    @pytest.mark.asyncio
    async def test_evaluate_assign_error(self):
        code = "a, b = 1, 2, 3; a"
        executor = LocalPythonExecutor([])
...
class TestLocalPythonExecutorSecurity:
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_import(self, additional_authorized_imports, expected_error):
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_builtins(self, additional_authorized_imports, expected_error):
        executor = LocalPythonExecutor(additional_authorized_imports)
        with (
            pytest.raises(type(expected_error), match=f".*{expected_error}")
            if isinstance(expected_error, Exception)
            ...
            await executor("import builtins")
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_builtins_safe_functions(self, additional_authorized_imports, expected_error):
        executor = LocalPythonExecutor(additional_authorized_imports)
        with (
            pytest.raises(type(expected_error), match=f".*{expected_error}")
            if isinstance(expected_error, Exception)
            ...
            await executor("import builtins; builtins.print(1)")
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_builtins_dangerous_functions(
        ...
        if additional_tools:
            ...
            await executor.send_tools({"exec": exec})
        with (
            pytest.raises(type(expected_error), match=f".*{expected_error}")
            if isinstance(expected_error, Exception)
            else does_not_raise()
        ):
            await executor("import builtins; builtins.exec")
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_dangerous_functions(self, additional_authorized_imports, additional_tools, expected_error):
        executor = LocalPythonExecutor(additional_authorized_imports)
        if additional_tools:
            from os import popen

            await executor.send_tools({"popen": popen})
        with (
            pytest.raises(type(expected_error), match=f".*{expected_error}")
            if isinstance(expected_error, Exception)
            else does_not_raise()
        ):
            await executor("import os; os.popen")

    @pytest.mark.parametrize("dangerous_function", DANGEROUS_FUNCTIONS)
    @pytest.mark.asyncio
    async def test_vulnerability_for_all_dangerous_functions(self, dangerous_function):
        ...
        with pytest.raises(InterpreterError, match=f".*Forbidden access to function: {dangerous_function_name}"):
            await executor(f"import {dangerous_module_name}; {dangerous_function}")
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_via_sys(self, additional_authorized_imports, expected_error):
        executor = LocalPythonExecutor(additional_authorized_imports)
        with (
            pytest.raises(type(expected_error), match=f".*{expected_error}")
            if isinstance(expected_error, Exception)
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_via_sys_for_all_dangerous_modules(self, dangerous_module):
        import sys
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_via_importlib(self, additional_authorized_imports, expected_error):
        executor = LocalPythonExecutor(additional_authorized_imports)
        with (
            pytest.raises(type(expected_error), match=f".*{expected_error}")
            if isinstance(expected_error, Exception)
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_via_submodules(self, code, additional_authorized_imports, expected_error):
        executor = LocalPythonExecutor(additional_authorized_imports)
        with (
            pytest.raises(type(expected_error), match=f".*{expected_error}")
            if isinstance(expected_error, Exception)
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_builtins_via_sys(self, additional_authorized_imports, additional_tools, expected_error):
        executor = LocalPythonExecutor(additional_authorized_imports)
        if additional_tools:
            from builtins import __import__

            await executor.send_tools({"__import__": __import__})
        with (
            pytest.raises(type(expected_error), match=f".*{expected_error}")
            if isinstance(expected_error, Exception)
            else does_not_raise()
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_builtins_via_traceback(
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_builtins_via_class_catch_warnings(
        self, patch_builtin_import_module, additional_authorized_imports, additional_tools, expected_error, monkeypatch
    ):
        if patch_builtin_import_module:
            monkeypatch.setattr("builtins.__import__.__module__", None)  # inspect.getmodule(func) = None
    ...
    @pytest.mark.asyncio
    async def test_vulnerability_load_module_via_builtin_importer(self, additional_authorized_imports, expected_error):
        executor = LocalPythonExecutor(additional_authorized_imports)
        with (
            pytest.raises(type(expected_error), match=f".*{expected_error}")
            if isinstance(expected_error, Exception)
