...
from .test_agent import TestDependencies, AnalysisResult
...
def hypha_server_url():
    ...
    return f"ws://127.0.0.1:{WS_PORT}/ws"
...
@pytest.mark.asyncio
async def test_agent_registration(hypha_server, test_agent, test_deps):
    """Test registering an agent as a Hypha service."""
    # Register the agent
    service_info = await test_agent.register(hypha_server, "test-agent")
    service = await hypha_server.get_service(service_info.id)
...
@pytest.mark.asyncio
async def test_agent_registration_with_tools(hypha_server, openai_model):
    ...
    # Register agent
    service_info = await agent.register(hypha_server, "tool-agent")
    service = await hypha_server.get_service(service_info.id)
...
@pytest.mark.asyncio
async def test_agent_registration_with_structured_output(hypha_server, structured_agent):
    """Test registering an agent with structured output as a Hypha service."""
    # Register agent
    service_info = await structured_agent.register(hypha_server, "structured-agent")
    service = await hypha_server.get_service(service_info.id)
...
@pytest.mark.asyncio
async def test_agent_registration_with_reasoning(hypha_server, openai_model):
    ...
    # Register agent
    service_info = await agent.register(hypha_server, "reasoning-agent")
    service = await hypha_server.get_service(service_info.id)
...
@pytest.mark.asyncio
async def test_agent_registration_client_usage(hypha_server, hypha_server_url, test_agent):
    ...
    service_info = await test_agent.register(hypha_server, service_id)
    service = await hypha_server.get_service(service_info.id)
