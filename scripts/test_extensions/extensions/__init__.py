...
def get_extension_by_name(name : str):
    extensions = []
    for module in pkgutil.walk_packages(__path__, __name__ + '.'):
        if module.name.endswith('_extension'):
            ext_module = module.module_finder.find_module(module.name).load_module(module.name)
            ...
            for ext in exts:
                if ext.name in [e.name for e in extensions]:
                    raise ValueError(f"Extension name {ext.name} already exists.")
            extensions.extend(exts)
    ext_hits = [ext for ext in extensions if ext.name == name]
    if len(ext_hits) == 0:
        raise ValueError(f"Extension name {name} does not exist.")
    elif len(ext_hits) > 1:
        raise ValueError(f"Extension name {name} is ambiguous.")
    return ext_hits[0]
