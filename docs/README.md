...
```bash
doc-builder build schema_agents docs/source/en/ --build_dir ~/tmp/test-build
```
...
```bash
doc-builder preview schema_agents docs/source/en/
```
...
the filename without the extension in the [`_toctree.yml`](https://github.com/huggingface/schema_agents/blob/main/docs/source/_toctree.yml) file.
...
The `huggingface/schema_agents` documentation follows the
