...
> [!TIP]
> 中文社区发布了关于 schema_agents 的介绍和实践讲解视频(来源：[Issue#80](https://github.com/huggingface/schema_agents/issues/80))，你可以访问[这里](https://www.youtube.com/watch?v=wwN3oAugc4c)进行观看！
...
```python
# !pip install schema_agents[transformers]
from schema_agents import CodeAgent, TransformersModel
...
```python
# !pip install schema_agents[litellm]
from schema_agents import CodeAgent, LiteLLMModel
...
```python
# !pip install schema_agents[openai]
from schema_agents import CodeAgent, AzureOpenAIServerModel
...
```python
# !pip install schema_agents[mlx-lm]
from schema_agents import CodeAgent, MLXModel
...
```py
from schema_agents import ToolCallingAgent
...
`schema_agents` 附带了一个用于增强 agent 的默认工具箱，您可以在初始化时通过参数 `add_base_tools = True` 将其添加到您的 agent 中：
...
您可以使用 `schema_agents` 轻松构建分层多 agent 系统。
...
```py
from schema_agents import CodeAgent, HfApiModel, DuckDuckGoSearchTool, ManagedAgent
