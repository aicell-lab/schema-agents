...
To add a first layer of security, code execution in `schema_agents` is not performed by the vanilla Python interpreter.
...
sandbox.commands.run("pip install schema_agents")
...
2. Install the required packages:
```bash
pip install 'schema_agents[docker]'
```
...
RUN apt-get update && \
    ...
    pip install --no-cache-dir schema_agents && \
    apt-get clean && \
