...
`schema_agents` comes with a default toolbox for empowering agents, that you can add to your agent upon initialization with argument `add_base_tools = True`:
...
You can easily build hierarchical multi-agent systems with `schema_agents`.
...
```py
from schema_agents import CodeAgent, HfApiModel, DuckDuckGoSearchTool
