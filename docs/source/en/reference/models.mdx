...
SchemaAgents is an experimental API which is subject to change at any time. Results returned by the agents
...
> [!TIP]
> You must have `transformers` and `torch` installed on your machine. Please run `pip install schema_agents[transformers]` if it's not the case.
...
```python
from schema_agents import MLXModel
...
> [!TIP]
> You must have `mlx-lm` installed on your machine. Please run `pip install schema_agents[mlx-lm]` if it's not the case.
