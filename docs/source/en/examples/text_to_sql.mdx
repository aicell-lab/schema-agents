...
In this tutorial, we’ll see how to implement an agent that leverages SQL using `schema_agents`.
...
```bash
!pip install schema_agents python-dotenv sqlalchemy --upgrade -q
```
...
We use the `CodeAgent`, which is schema_agents’ main agent class: an agent that writes actions in code and can iterate on previous output according to the ReAct framework.
