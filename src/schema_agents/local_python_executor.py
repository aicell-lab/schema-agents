...
class LocalPythonExecutor(PythonExecutor):
    def __init__(
        ...
        # Create a thread pool executor for running code evaluation
        self._executor = concurrent.futures.ThreadPoolExecutor()

    async def __call__(self, code_action: str) -> Tuple[Any, str, bool]:
        """
        Execute Python code asynchronously using a thread pool to prevent blocking the main thread.
        ...
        try:
            # Run the evaluation in a separate thread using the executor
            ...
            if isinstance(e, InterpreterError):
                # For interpreter errors, just pass them through
                raise
            else:
                # For other errors, wrap them in an InterpreterError
                raise InterpreterError(f"Async execution error: {type(e).__name__}: {str(e)}") from e
