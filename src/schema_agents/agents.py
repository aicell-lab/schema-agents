...
class MultiStepAgent:
    ...
    async def run(
        ...
        Example:
        ```py
        from schema_agents import CodeAgent
...
{str(additional_args)}."""
        ...
        if getattr(self, "python_executor", None):
            await self.python_executor.send_variables(variables=self.state)
            await self.python_executor.send_tools({**self.tools, **self.managed_agents})

        if stream:
            # The steps are returned as they are executed through a generator to iterate on.
            return self._run(task=self.task, max_steps=max_steps, images=images)
        
        # For non-streaming mode, consume the generator and return the last item
        last_result = None
        async for result in self._run(task=self.task, max_steps=max_steps, images=images):
            last_result = result
        return last_result
    ...
    async def _execute_step(self, task: str, memory_step: ActionStep) -> Union[None, Any]:
        ...
        if final_answer is not None and self.final_answer_checks:
            await self._validate_final_answer(final_answer)
        return final_answer

    async def _validate_final_answer(self, final_answer: Any):
    ...
    async def planning_step(self, task, is_first_step: bool, step: int) -> None:
        input_messages, facts_message, plan_message = (
            await self._generate_initial_plan(task) if is_first_step else await self._generate_updated_plan(task, step)
        )
        await self._record_planning_step(input_messages, facts_message, plan_message, is_first_step)

    async def _generate_initial_plan(self, task: str) -> Tuple[ChatMessage, ChatMessage]:
        input_messages = [
            {
    ...
    async def _generate_updated_plan(self, task: str, step: int) -> Tuple[ChatMessage, ChatMessage]:
        ...
        memory_messages = await self.write_memory_to_messages(summary_mode=False)
        facts_update_pre = {
        ...
        try:
            if isinstance(arguments, str):
                if tool_name in self.managed_agents:
                    ...
                    # If the tool is not async, just call it
                    observation = available_tools[tool_name].__call__(arguments, sanitize_inputs_outputs=True)
                    # If the result is awaitable, await it (for backward compatibility)
    ...
    async def save(self, output_dir: str, relative_path: Optional[str] = None):
        ...
        if self.managed_agents:
            ...
            for agent_name, agent in self.managed_agents.items():
                ...
                await agent.save(os.path.join(output_dir, "managed_agents", agent_name), relative_path=agent_suffix)
        ...
        for tool in self.tools.values():
            ...
            await tool.save(os.path.join(output_dir, "tools"), tool_file_name=tool.name, make_gradio_app=False)
        ...
        app_template = textwrap.dedent("""
            import yaml
            import os
            from schema_agents import GradioUI, {{ class_name }}, {{ agent_dict['model']['class'] }}
    ...
    def from_folder(cls, folder: Union[str, Path], **kwargs):
        ...
        for managed_agent_name, managed_agent_class in agent_dict["managed_agents"].items():
            agent_cls = getattr(importlib.import_module("schema_agents.agents"), managed_agent_class)
        ...
        model_class: Model = getattr(importlib.import_module("schema_agents.models"), agent_dict["model"]["class"])
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("schema_agents.prompts").joinpath("toolcalling_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
...
class CodeAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("schema_agents.prompts").joinpath("code_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
