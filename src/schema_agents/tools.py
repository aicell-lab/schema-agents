...
class Tool:
    ...
    async def async_forward(self, *args, **kwargs):
        """
        Async version of the forward method. By default, it calls the synchronous forward method.
        Override this method to implement async functionality.
        """
    ...
    async def __call__(self, *args, sanitize_inputs_outputs: bool = False, **kwargs):
        ...
        outputs = await self.async_forward(*args, **kwargs)
        if sanitize_inputs_outputs:
    ...
    def to_dict(self) -> dict:
        """Converts tool into a dictionary."""
        source_code = get_source(self.__class__)
        if not source_code:
            source_code = instance_to_source(self)
        imports = get_imports(source_code)
        requirements = []
        for import_name in imports:
            if import_name in BASE_BUILTIN_MODULES:
                continue
    ...
    async def save(self, output_dir: str, tool_file_name: str = "tool", make_gradio_app: bool = True):
        ...
        tool_code = tool_dict["source_code"]
        ...
        if make_gradio_app:
            ...
            with open(app_file, "w", encoding="utf-8") as f:
                f.write(
                    textwrap.dedent(
                        f"""
                from schema_agents import launch_gradio_demo
    ...
    async def push_to_hub(
        ...
        metadata_update(repo_id, {"tags": ["schema_agents", "tool"]}, repo_type="space", token=token)
    ...
    async def from_hub(
        ...
        **kwargs,
        ...
        # Get the tool file.
        try:
            tool_file = hf_hub_download(repo_id=repo_id, filename="tool.py", **download_kwargs)
        except Exception:
            tool_file = hf_hub_download(repo_id=repo_id, filename="app.py", **download_kwargs)

        # Load the tool from the file.
        return cls.from_code(open(tool_file, "r", encoding="utf-8").read(), **kwargs)

    @classmethod
    def from_code(cls, tool_code: str, **kwargs):
        """
        Loads a tool from its code.

        <Tip warning={true}>

        Loading a tool from code means that you'll execute it locally.
        ALWAYS inspect the code you're loading before executing it within your runtime, as you would do when
        installing a package using pip/npm/apt.
        ...
        Args:
            tool_code (`str`):
                The code of the tool to load.
            kwargs (additional keyword arguments, *optional*):
                Additional keyword arguments that will be passed along to the tool's init.
        """
        # Create a module for the tool.
        module = types.ModuleType("tool_module")
        
        # Add the Tool class to the module's namespace
        module.__dict__["Tool"] = cls
        
        # Extract imports from the tool code
        try:
            ...
            imports.extend(["typing", "typing.Any", "typing.Dict", "typing.List", "typing.Optional", "typing.Union"])
            
            # Add all base builtin modules
            imports.extend(BASE_BUILTIN_MODULES)
            
            # Import all detected modules
            for import_name in set(imports):
                try:
                    # Handle typing special cases
                    if import_name.startswith("typing."):
                        attr_name = import_name.split(".")[-1]
                        from typing import Any, Dict, List, Optional, Union, Callable, Tuple, Set, TypeVar, Generic, Protocol, cast, overload
                        typing_attrs = {
                            "Any": Any,
                            "Dict": Dict,
                            "List": List,
                        ...
                        if attr_name in typing_attrs:
                            module.__dict__[attr_name] = typing_attrs[attr_name]
                    else:
                        # Try to import the module directly
                        base_module_name = import_name.split(".")[0]
                        imported_module = importlib.import_module(base_module_name)
                        module.__dict__[base_module_name] = imported_module
                        
                        # If it's a submodule, try to import it too
                        if "." in import_name:
                            try:
                                submodule = importlib.import_module(import_name)
                                parts = import_name.split(".")
                                module.__dict__[parts[-1]] = submodule
                            except ImportError:
                                pass
                except ImportError:
            ...
            # If parsing fails, fall back to basic imports
            logger.warning("Failed to parse tool code for imports, falling back to basic imports")
            ...
            # Import all base builtin modules
            for builtin_module_name in BASE_BUILTIN_MODULES:
                try:
                    builtin_module = importlib.import_module(builtin_module_name)
                    module.__dict__[builtin_module_name] = builtin_module
                except ImportError:
                    pass
        
        # Execute the tool code in the prepared module
        try:
            exec(tool_code, module.__dict__)
        except NameError as e:
            # If we encounter a NameError, try to import the missing name
            missing_name = str(e).split("'")[1] if "'" in str(e) else None
            if missing_name:
                try:
                    # Try to import as a module
                    imported_module = importlib.import_module(missing_name)
                    module.__dict__[missing_name] = imported_module
                    # Try again with the new import
                    exec(tool_code, module.__dict__)
        ...
        if tool_class is None:
            raise ValueError(f"Could not find a subclass of {cls.__name__} in the provided code.")
...
class ToolCollection:
    ...
    def from_hub(
        ...
        Example:
        ```py
        >>> from schema_agents import ToolCollection, CodeAgent
    ...
    def from_mcp(cls, server_parameters) -> "ToolCollection":
        ...
        try:
            ...
            raise ImportError(
                """Please install 'mcp' extra to use ToolCollection.from_mcp: `pip install "schema_agents[mcp]"`."""
            )
        ...
        **hub_kwargs,
        ...
        if not is_torch_available() or not _is_package_available("accelerate"):
            raise ModuleNotFoundError(
                "Please install 'transformers' extra to use a PipelineTool: `pip install 'schema_agents[transformers]'`"
            )
...
def get_tools_definition_code(tools: Dict[str, Tool]) -> str:
    ...
    for tool in tools.values():
        ...
        tool_code = tool_code.replace("from schema_agents.tools import Tool", "")
